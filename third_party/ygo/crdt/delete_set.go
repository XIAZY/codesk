	...
	for client, ranges := range ds.clients {
		for _, r := range ranges {
			...
			for start < end {
				...
				// Find the first item whose end is after the next undeleted
				// clock. The delete range may begin inside a larger text item,
				// so the item must be split before it is tombstoned.
				i := sort.Search(len(items), func(i int) bool {
					return items[i].ID.Clock+uint64(items[i].Content.Len()) > start
				})
				if i >= len(items) {
					break
				}
				item := items[i]
				itemStart := item.ID.Clock
				...
				if itemStart < start {
					item = splitItem(txn, item, int(start-itemStart))
					itemStart = item.ID.Clock
					itemEnd = item.ID.Clock + uint64(item.Content.Len())
				}
				if itemEnd > end {
					splitItem(txn, item, int(end-itemStart))
					itemEnd = end
				}
				item.delete(txn)
				start = itemEnd
