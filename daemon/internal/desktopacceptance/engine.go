...
func (e Engine) run(
	...
	*publishable = candidateTrust == StatusPass && previousTrust == StatusPass
	...
	if err := e.step(ctx, recorder, "legacy-launcher-fixture", func(stepCtx context.Context) (string, error) {
		expected, err := e.Adapter.SeedLegacyLaunchers(stepCtx)
		if err != nil {
			return "", err
		}
		if expected <= 0 {
			return "", errors.New("native adapter did not define a legacy-launcher fixture")
		}
		state, err := e.Adapter.Snapshot(stepCtx)
		if err != nil {
			return "", err
		}
		if len(state.LegacyLaunchers) != expected {
			return "", fmt.Errorf("native adapter exposed %d legacy launchers after seeding, want %d", len(state.LegacyLaunchers), expected)
		}
		return fmt.Sprintf("seeded=%d", expected), nil
	}, config.Timeout); err != nil {
		return err
	}
	...
	if err := e.step(ctx, recorder, "candidate-upgrade-install", func(stepCtx context.Context) (string, error) {
		...
		state, err := e.waitState(stepCtx, func(state State) bool {
			return installedState(state, config.Candidate.Version) == nil && len(state.LegacyLaunchers) == 0
		})
		if err != nil {
			return "", err
		}
		...
		return "exact candidate installed; registration replaced; legacy launchers removed; configuration and credential preserved", nil
	}, config.Timeout); err != nil {
		return err
	}
...
func installedState(state State, version string) error {
	...
	if len(state.Application) != 0 || len(state.ManagedDescendants) != 0 {
		return errors.New("--no-launch setup unexpectedly left resident processes")
	}
	return nil
}
...
func releaseTrust(release Release, allowUnsigned bool) (Status, string, error) {
	...
	for _, artifact := range release.Artifacts {
		...
		if artifact.SignatureValid {
			return StatusFail, "", fmt.Errorf("%s %s artifact is trusted by the native signature verifier but the manifest claims unsigned", release.Version, artifact.Architecture)
		}
		unsigned = true
	}
	if unsigned {
