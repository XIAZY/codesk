...
func (a *fakeAdapter) VerifyRelease(_ context.Context, input ReleaseInput) (Release, error) {
	...
	release := Release{
		...
		CrossArchitecturePolicy: "converge",
		Signed:                  signed,
	...
	for index, architecture := range []string{"amd64", "arm64"} {
		release.Artifacts = append(release.Artifacts, ReleaseArtifact{
			...
			Path:               filepath.Join(input.Directory, fmt.Sprintf("CodeskMSI_%s_windows_%s.msi", input.Version, architecture)),
			SHA256:             strings.Repeat(string(rune('a'+index)), 64),
			Size:               1024,
			ProductCode:        fmt.Sprintf("{%08d-1111-4111-8111-111111111111}", index+1),
			CodeskSHA256:       strings.Repeat("c", 64),
			AgentToolSHA256:    strings.Repeat("f", 64),
