...
func TestWorkspaceRuntimeMaterializesThreadIntentAfterAcceptedLocalBeforePendingIncoming(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "doc.md")
	if err := os.WriteFile(path, []byte("base\nlocal target\n"), 0o644); err != nil {
		t.Fatalf("write file: %v", err)
	...
	intent := threadOutboxIntent{
		IntentID:       "threadintent_local_before_incoming",
		IdempotencyKey: "threadintent_local_before_incoming",
		DocumentID:     "doc_1",
		DocumentPath:   "doc.md",
		ActorID:        "agent_1",
		ActorType:      "agent",
	...
	if count, err := cache.pendingRemoteUpdateCount("doc_1"); err != nil {
		t.Fatalf("pending incoming count: %v", err)
	} else if count != 1 {
		t.Fatalf("accepted local update must not fold pending incoming yet, got %d", count)
	}
	...
	if intents[0].Status != threadIntentReady || intents[0].Resolved == nil {
		t.Fatalf("expected local quote to materialize before pending incoming, got %#v", intents[0])
	}
	...
	if count, err := cache.pendingRemoteUpdateCount("doc_1"); err != nil {
		t.Fatalf("pending incoming count after inbox: %v", err)
	} else if count != 0 {
		t.Fatalf("expected second pass to fold pending incoming after materialization, got %d", count)
	}
}
