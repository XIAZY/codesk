...
func TestWorkspaceRuntimeMaterializesThreadIntentAfterAcceptedLocalBeforeEarlierPendingRemote(t *testing.T) {
	root := t.TempDir()
	path := filepath.Join(root, "doc.md")
	if err := os.WriteFile(path, []byte("base\nlocal target\n"), 0o644); err != nil {
		t.Fatalf("write file: %v", err)
	...
	intent := threadOutboxIntent{
		IntentID:       "threadintent_local_before_remote",
		IdempotencyKey: "threadintent_local_before_remote",
		DocumentID:     "doc_1",
		DocumentPath:   "doc.md",
		ActorID:        "agent_1",
		ActorType:      "agent",
	...
	if count, err := cache.pendingRemoteUpdateCount("doc_1"); err != nil {
		t.Fatalf("pending remote count: %v", err)
	} else if count != 1 {
		t.Fatalf("accepted local update must not fold pending remote yet, got %d pending remote updates", count)
	}

	if err := runtime.reconcileTrackedDocument(context.Background(), "doc_1", []*trackedFile{tracked}); err != nil {
		t.Fatalf("second reconcile materialize thread before inbox: %v", err)
	}
	intents := loadThreadIntentsForTest(t, cache, "doc_1")
	if len(intents) != 1 {
		t.Fatalf("expected one thread intent, got %#v", intents)
	}
	if intents[0].Status != threadIntentReady || intents[0].Resolved == nil {
		t.Fatalf("expected local quote to materialize before earlier pending remote, got %#v", intents[0])
	}
	if intents[0].Resolved.Excerpt != "local target" || intents[0].Resolved.RelativeStart == "" || intents[0].Resolved.RelativeEnd == "" {
		t.Fatalf("unexpected resolved thread payload: %#v", intents[0].Resolved)
	}
	if count, err := cache.pendingRemoteUpdateCount("doc_1"); err != nil {
		t.Fatalf("pending remote count after inbox: %v", err)
	} else if count != 0 {
		t.Fatalf("expected second pass to fold pending remote after materialization, got %d", count)
	}
}
