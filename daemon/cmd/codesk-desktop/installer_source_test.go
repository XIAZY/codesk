...
func TestWindowsInstallerReproducibilityScriptsAreFailClosed(t *testing.T) {
	...
	mutations := []struct {
		...
		},
		{
			...
			verifyOld: "$First.SourceText -cne $Second.SourceText",
			verifyNew: "$First.SourceText -cne $First.SourceText",
		},
		{
			name:      "database comparison disabled",
			verifyOld: "$First.DatabaseFilesJson -cne $Second.DatabaseFilesJson",
			verifyNew: "$First.DatabaseFilesJson -cne $First.DatabaseFilesJson",
		},
		{
			name:      "extracted payload comparison disabled",
			verifyOld: "$First.ExtractedFilesJson -cne $Second.ExtractedFilesJson",
			verifyNew: "$First.ExtractedFilesJson -cne $First.ExtractedFilesJson",
		},
		{
...
func checkWindowsInstallerReproducibilityScripts(build, verify string) error {
	...
	for _, required := range []string{
		...
		"$First.SourceText -cne $Second.SourceText",
		"$First.DatabaseFilesJson -cne $Second.DatabaseFilesJson",
		"$First.ExtractedFilesJson -cne $Second.ExtractedFilesJson",
