...
func (s *Server) publishActivityChanges(r *http.Request) {
	publishActivityChanges(s.requestStore(r), s.requestBroker(r))
}
...
func publishActivityChanges(store *Store, broker *Broker) {
	if store == nil || broker == nil {
		return
	}
	for _, activity := range store.DrainActivityChanges() {
		broker.Publish(EventEnvelope{Type: "activity.created", Data: activity})
	}
}
