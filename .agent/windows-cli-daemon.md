...
- [x] (2026-07-13 06:18Z) Kept Windows as an explicit `PLATFORMS=windows/amd64` construction target rather than adding it to the public `all` release set. Production must not advertise an experimental Windows artifact or acquire Windows cross-toolchain prerequisites until native and user-facing packaging acceptance is complete.
...
- Observation: the Win32 `FILE_RENAME_INFO` buffer requires a trailing UTF-16 NUL even though `FileNameLength` excludes it.
  Evidence: omitting the terminator passed the single-row probe but intermittently returned `ERROR_INVALID_NAME` under repeated concurrent scan/write stress; an ABI row now pins the 64-bit field offsets, buffer size, excluded length, and terminator.

- Observation: an atomic namespace swap is not enough for the daemon's projected-base ordering. If replacement returns before complete staging bytes and the rename metadata are flushed, a crash can leave the persisted projected base ahead of disk; restart then classifies stale disk bytes as a local edit.
  Evidence: the replacement path previously wrote/chmodded/closed without `Sync`, and the first `FileRenameInfoEx` prototype reopened with `DELETE` only and returned without `FlushFileBuffers`. Failure-injection rows now require pre-rename sync and post-rename flush to succeed before `WriteIfUnchanged` returns or the projected base advances.
...
- Decision: Windows atomic replacement on the P1 support baseline (Windows 10 v1607+ on NTFS) syncs the complete sibling staging file, then uses a write/delete handle with `FileRenameInfoEx` replace/POSIX semantics and `FlushFileBuffers` before returning success. Daemon scan observations opt into delete sharing; externally held destinations that deny it and unsupported filesystems fail visibly, retain complete old content when commit has not occurred, and clean staging.
  Rationale: `MoveFileEx` cannot perform the required namespace swap while a scan observation is open. POSIX rename semantics explicitly keep old handles valid while future opens resolve to the replacement object, while the two flush gates prevent projected-base persistence from advancing over a non-durable projection. A fallback to `MoveFileEx` would reintroduce the original open-observation failure, so unsupported filesystems are a visible safe failure rather than a weaker silent path.
  Date/Author: 2026-07-13, AlphaToad/Vitaliy, amended after native Windows red, stress evidence, and durability review.
...
Move atomic replacement to `file_replace_unix.go` and `file_replace_windows.go`. Shared staging writes and chmods the complete file, calls `Sync`, then closes before platform commit. Unix retains rename. Windows reopens the staging file with write/delete access and commits it through `FileRenameInfoEx` with replace/POSIX semantics so delete-shared observations retain the old object while new opens resolve to the replacement, then calls `FlushFileBuffers` before returning success. Sharing violations and unsupported filesystem semantics must return a visible error; never truncate the destination first or advance the projected base after either durability gate fails. Windows tests cover destination absent, destination present, destination open with and without delete sharing, pre-sync and post-rename failure ordering, safe unsupported-commit cleanup, the exact 64-bit UTF-16 rename-info ABI, and unchanged/complete content after failure.
...
Revision note (2026-07-13): amended the handle-based replacement after review found that the first prototype could return before staging data and rename metadata were durable. Added pre-rename `Sync`, a write-capable rename handle plus post-rename `FlushFileBuffers`, failure-ordering coverage that keeps projected-base persistence behind both gates, explicit supported-64-bit ABI offsets, and a visible safe-failure contract for non-NTFS filesystems.
