...
The final lifecycle audit materially improved the first draft. It added explicit release of naturally completed child contexts and corrected two platform-specific shutdown cycles: Windows' synchronous `Quit` callback and Darwin's synchronous main-thread menu dispatch. The permanent controller matrix covers startup, readiness, retry cap/reset, reconnect, manual restart, join ordering, parent cancellation, atomic pre-start Quit, concurrent Restart/Quit pressure, and resource release. On the final rebased production tree, controller race 50x and focused syncer readiness plus real 401/403/410 `Service.Run` rows race 20x are green. Full `go test ./...`, daemon vet, daemon build, formatting, and diff checks passed on the first rebased production tree; after current main and the host Go toolchain advanced, the fresh full run reached only the identically reproduced pristine-main PR #162 fixture failures documented above.
