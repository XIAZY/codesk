...
# Create a minimal fake repo: just enough for VERSION guards to fire before any
# build tool (Go, Rust, zig) is invoked.
setup_fake_repo() {
	rm -rf "$tmp_dir/fake"
...
resolved="$(VERSION=injected make -C "$tmp_dir/fake" -n -p 2>/dev/null \
	| grep '^VERSION := ' | head -1 | sed 's/^VERSION := //')" || true
...
# ── Makefile: GUI_VERSION := cannot be overridden by env ──
resolved="$(GUI_VERSION=injected make -C "$tmp_dir/fake" -n -p 2>/dev/null \
	| grep '^GUI_VERSION := ' | head -1 | sed 's/^GUI_VERSION := //')" || true
if [ "$resolved" = "injected" ]; then
	fail "Makefile GUI_VERSION was overridden by environment variable"
fi
pass "Makefile GUI_VERSION := ignores environment override"
...
if command -v pwsh >/dev/null 2>&1; then
	...
	# ── make.ps1: GUI_VERSION is not a supported setting ──
	setup_fake_repo
	cp "$repo_dir/make.ps1" "$tmp_dir/fake/"
	printf '1.2.3\n' > "$tmp_dir/fake/VERSION"
...
if printf '%s' "$macos_gui_line" | grep -q '"$(GUI_VERSION)".*"$(MACOS_GUI_DIST_DIR)"'; then
