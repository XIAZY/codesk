...
$versionFile = Join-Path (Split-Path -Parent $PSScriptRoot) 'DAEMON_VERSION'
if (-not (Test-Path -LiteralPath $versionFile -PathType Leaf)) {
    throw "DAEMON_VERSION must be a regular file: $versionFile"
}

[byte[]] $bytes = [System.IO.File]::ReadAllBytes($versionFile)
if ($bytes.Length -lt 2 -or $bytes[$bytes.Length - 1] -ne 10) {
    throw 'DAEMON_VERSION must end with exactly one LF or CRLF'
}
...
for ($index = 0; $index -lt $lineLength; $index++) {
    if ($bytes[$index] -eq 10 -or $bytes[$index] -eq 13) {
        throw 'DAEMON_VERSION must contain exactly one LF- or CRLF-terminated line'
    }
}

$version = [System.Text.Encoding]::ASCII.GetString($bytes, 0, $lineLength)
if ($version -cnotmatch '^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$') {
    throw 'DAEMON_VERSION must be canonical X.Y.Z without whitespace or leading zeros'
}
...
if (-not [uint32]::TryParse($fields[0], $style, $culture, [ref] $major) -or
    ...
    throw 'DAEMON_VERSION contains a field outside the unsigned integer domain'
}
if ($major -gt 255 -or $minor -gt 255 -or $build -gt 65535) {
    throw 'DAEMON_VERSION exceeds MSI limits (major/minor <= 255, build <= 65535)'
}
